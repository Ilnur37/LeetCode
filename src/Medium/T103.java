...
public class T103 {
    ...
    public static void checkChildren(TreeNode node, int lvl, List<List<Integer>> tr) {
        ...
        if (node.left != null) {
            if (lvl % 2 != 0) tr.get(lvl).add(0, node.left.val);
            else tr.get(lvl).add(node.left.val);
            checkChildren(node.left, ++lvl, tr);
            lvl--;

        }
        if (node.right != null) {
            if (lvl % 2 != 0) tr.get(lvl).add(0, node.right.val);
            else tr.get(lvl).add(node.right.val);
